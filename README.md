...
> **Note**: The module supports both x86\_64 (Intel/AMD) and ARM64 (Graviton) architectures. The correct Falcon loader is automatically selected based on the `runtime_platform.cpu_architecture` setting.
...
| <a name="input_app_cpu"></a> [app\_cpu](#input\_app\_cpu) | CPU units to allocate to the application container (not the entire task). If not specified, no container-level CPU limit is set. | `number` | `null` | no |
| <a name="input_app_entrypoint"></a> [app\_entrypoint](#input\_app\_entrypoint) | The entrypoint for the application container (will be wrapped by Falcon). If not specified, Falcon will attempt to wrap the image's default entrypoint. For best results, explicitly specify the entrypoint (e.g., ["/docker-entrypoint.sh"] for nginx). | `list(string)` | `null` | no |
...
| <a name="input_app_log_configuration"></a> [app\_log\_configuration](#input\_app\_log\_configuration) | Log configuration for the application container. If not specified, defaults to awslogs via enable\_logging/log\_group\_name. | <pre>object({<br/>    logDriver = string<br/>    options   = optional(map(string), {})<br/>    secretOptions = optional(list(object({<br/>      name      = string<br/>      valueFrom = string<br/>    })), [])<br/>  })</pre> | `null` | no |
| <a name="input_app_memory"></a> [app\_memory](#input\_app\_memory) | Memory (in MB) to allocate to the application container (hard limit). If not specified, no container-level memory limit is set. | `number` | `null` | no |
| <a name="input_app_memory_reservation"></a> [app\_memory\_reservation](#input\_app\_memory\_reservation) | Soft memory limit (in MB) for the application container. Container will try to stay below this limit. | `number` | `null` | no |
...
| <a name="input_app_stop_timeout"></a> [app\_stop\_timeout](#input\_app\_stop\_timeout) | Time duration (in seconds) to wait before the container is forcefully killed if it doesn't exit normally on its own. ECS default is 30 seconds. | `number` | `null` | no |
...
| <a name="input_enable_execute_command"></a> [enable\_execute\_command](#input\_enable\_execute\_command) | Enable ECS Exec for debugging (allows running commands in containers). Requires task role with SSM permissions. | `bool` | `false` | no |
...
| <a name="input_falcon_init_cpu"></a> [falcon\_init\_cpu](#input\_falcon\_init\_cpu) | CPU units to allocate to the Falcon init container | `number` | `256` | no |
| <a name="input_falcon_init_memory"></a> [falcon\_init\_memory](#input\_falcon\_init\_memory) | Memory (in MB) to allocate to the Falcon init container | `number` | `512` | no |
| <a name="input_falcon_init_timeout"></a> [falcon\_init\_timeout](#input\_falcon\_init\_timeout) | Timeout in seconds for the Falcon init container to complete | `number` | `120` | no |
...
| <a name="input_log_group_kms_key_id"></a> [log\_group\_kms\_key\_id](#input\_log\_group\_kms\_key\_id) | ARN of the KMS key to use for encrypting CloudWatch log data. If not specified, logs will use default encryption. | `string` | `null` | no |
...
| <a name="input_platform_version"></a> [platform\_version](#input\_platform\_version) | Fargate platform version to use for the ECS service. Use this value when creating your ECS service. Defaults to LATEST. | `string` | `"LATEST"` | no |
...
| <a name="output_platform_version"></a> [platform\_version](#output\_platform\_version) | Fargate platform version to use (use this value when creating ECS service) |
